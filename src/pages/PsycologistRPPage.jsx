...
import { CircleX, Instagram, Check, FileInput , Clapperboard, Image } from "lucide-react";
...
import PopupPsyAnketaRP from "@/components/PopupPsyAnketaRP";
import EduList from "@/components/EduList";
import axios from "axios";
import TextArea from "@/components/TextArea";


const PsycologistRPPage = () => {
  ...
  const uploaderSupervisionConfirmation = useMemo(() => ({
    [UPLOADER_EVENTS.ITEM_START]: (item) => {
      console.log(`Item Start - ${item.id} : ${item.file.name}`);
      setUploadSupervisionConfirmation('loading')
    },
    [UPLOADER_EVENTS.ITEM_FINISH]: (item) => {
      console.log(`Item Finish - ${item.id} : ${item.file.name}`);
      setUploadSupervisionConfirmation('success')
    },
  }), []);

  const [uploadSupervisionConfirmation, setUploadSupervisionConfirmation] = useState('empty')
  ...
  function validateFields(form) {
    ...
    emptyKeys = removeElementAtValue(emptyKeys, "city");
    emptyKeys = removeElementAtValue(emptyKeys, "sexClient");
  ...
  useEffect(() => {
    ...
    axios(
      {
        url: "https://n8n-v2.hrani.live/webhook/has-supervision-docs",
        params: { psychologist_id }
      }
    ).then(
      resp => {
      ...
      <div className="bg-dark-green w-full h-full flex flex-col justify-center items-center">
        ...
        <div data-name="container" className="max-w-[800px]">
          <div data-name="header" className="flex flex-col items-center">
            ...
            >
              Анкета психолога проекта<br/>«Рука помощи»
            </h1>
            <p
          ...
          <div data-name="survey" className="flex flex-col gap-14 px-5">
            ...
            <div data-name="question" className="flex flex-col gap-4">
              ...
              <TextArea
                placeholder='Введите информацию в раздел "О Психологе" на сайте - вы можете сказать пару слов своими словами о том, почему начали заниматься психологией, что для вас клиенты, почему нравится помогать людям, какой у вас стиль (чуткий или  прямолинейный, вы ближе к эзотерике или научному подходу и тд), отметьте модальности которые изучаете и техники, которые используете, к чему хотите прийти сами и привести клиента'
                intent="cream"
                rows={7}
            ...
            <div data-name="question" className="flex flex-col gap-4">
              ...
              <ul
                className={`flex flex-col gap-2 p-2 ${showErrorBorder && (isEmpty(checkedMainModal) && anketa.customMainModal == '')
              ...
              >
                ...
                <li>
                  <Input
                    ...
                    intent="cream"
                    value={anketa.customMainModal}
                    onChangeFn={(e) => dispatch(setCustomMainModal(e))}
                  ></Input>
                </li>
              </ul>
            </div>
            ...
            <div data-name="question" className="flex flex-col gap-6">
              ...
              <ul className={`flex flex-col gap-2 p-2 ${showErrorBorder && (isEmpty(checkedAdditionalModals) && anketa.customAdditionalModals == '')
                ? "border-red border rounded-[15px]"
                : ""
                }`}>
                ...
                {additional_modal_list.map((ad_modal, index) => (
                  <li>
                    ...
                    <Radio
                      ...
                      intent="cream"
                      id={`ad_modal_${index}`}
                      onChange={() => dispatch(setAdditionalModals(ad_modal))}
                ...
                <li>
                  <Input
                    placeholder="Свой вариант"
                    intent="cream"
                    value={anketa.customAdditionalModals}
                    onChangeFn={(e) => dispatch(setCustomAdditionalModals(e))}
                  ></Input>
                </li>
              </ul>
            </div>
            ...
            <div data-name="question" className="flex flex-col gap-6">
              <div data-name="header" className="flex gap-4 items-start">
                ...
                <div className="flex flex-col gap-1">
                  <h2 className="text-corp-white font-bold text-2xl font-sans">
                    Загрузите фото, видео и документ о прохождении супервизии
                  </h2>
                  <p className="text-corp-white text-sm">
                    Для размещения информации о вас на сайте Хранителей необходимо загрузить фотографию. Видеовизитку вы может загрузить позднее. Рекомендации по фото и видео <a className="text-cream underline" href="https://docs.google.com/document/d/1hH5DS4xP7URaHl_z-VCQNxUMpoLCZAmi_VhVLEAvXwE/edit?tab=t.0" target="_new">можно найти в этом файле</a>. Подтверждением посещения супервизий может быть сертификат или письмо в свободной форме от вашего супервизора с указанием места прохождения супервизий и/или ФИО супервизора и количества часов. Вы можете выслать этот документ администратору позднее
                  </p>
                </div>
              </div>
              <div
              ...
              >
                ...
                <Uploady
                  listeners={uploaderSupervisionConfirmation}
                  destination={{ url: `https://n8n-v2.hrani.live/webhook/upload-psychologist-supervision-docs?psychologist=${psychologist_id}&psychologist_name=${anketa.name}` }}
                >

                  {uploadSupervisionConfirmation == 'empty' && <UploadButton text="Выберите документ (необязательно)" className="p-4 text-cream border-2 border-cream border-dashed rounded-xl flex gap-2 items-center justify-center"><FileInput size={24} color="
                  #d9b08c"/>
                    <p>Выберите документ (необязательно)</p>
                  </UploadButton>}
                  {uploadSupervisionConfirmation == 'loading' &&
                    <div className="p-4 text-cream border-2 border-cream border-dashed rounded-xl flex gap-2 justify-center items-center">
                      <svg
                        width={24}
                        height={24}
                      ...
                      <p className="text-cream">документ загружается</p>
                    </div>}
                  {uploadSupervisionConfirmation == 'success' &&
                    <div className="p-4 text-cream border-2 border-cream border-dashed rounded-xl flex gap-2 items-center justify-center"><Check size={24} color="
                  #d9b08c"/>
                      <p>документ загружен</p>
                      <p onClick={() => { setUploadSupervisionConfirmation('empty') }} className="cursor-pointer text-white underline">Заменить</p>
                    </div>}
                </Uploady>
...
export default PsycologistRPPage;
