...
const Slots = () => {
  ...
  function selectFn(date) {
    ...
    function ignoreSlotsByPsychologist(filtered_groups, psyName){
      
      let psychologist_ignore = false
      let copy_filtered_groups = JSON.parse(JSON.stringify(filtered_groups))
          for (let group of filtered_groups) {
            for (let gs in group.slots) {
              let temp_slots = []
        ...
        if (psychologist_ignore){
          
          return [psychologist_ignore, filtered_groups]
        } else{
          return [psychologist_ignore, copy_filtered_groups]
        }
    }

    axios({
      method: "GET",
      url: "https://n8n-v2.hrani.live/webhook/get-ticket-info-for-reassigment",
      params: { lead_number,  mode},
    })
      .then((resp) => {
        let slots=resp.data.schedule
        let utm_psy = ''
        let ignore_psychologist = ''

        if(mode == 'time'){
          utm_psy = resp.data.psychologist
        }
        else if (mode == 'psychologist'){
          ignore_psychologist = resp.data.psychologist
        }
          ...
          for (let group of filtered_groups_copy) {
            for (let s in group.slots) {
              for (let p of group.slots[s]) {
                if (ignore_psychologist == p?.psychologist && mode=='psychologist'){
                  continue
                }
        ...
        // console.log('filtered_groups_copy', filtered_groups_copy)  
        
        if(mode=='psychologist'){
          
          let [ignore, fg] = ignoreSlotsByPsychologist(JSON.parse(JSON.stringify(filtered_groups_copy)), ignore_psychologist)
          filtered_groups_copy = fg

          console.log('filtered_groups_copy', filtered_groups_copy)
          //Удаляем первые пустые группы слотов если задан конкретный психолог 
          let found = false
          let f_index = 0
          for(let g of filtered_groups_copy){
          let gs = g.slots
            for(let time in gs){
              if (gs[time].length !=0){
                found = true
        ...
        console.log(filtered_groups_copy[1])

        console.log('slots_by_psycho',  slots_by_psycho, filtered_groups )
        // if ((checkNoSlots(filtered_groups) && mode == 'time') || (checkNoSlots(filtered_groups_copy) && mode == 'psychologist')) {
        if ((checkNoSlots(filtered_groups) && mode == 'time') || (checkNoSlots(filtered_groups) && mode == 'psychologist')) {
